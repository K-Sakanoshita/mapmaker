...
class GeoCoastline {
	merge2(geojson_s) {
		var coastlineFeatures = geojson_s.filter(geojson => {
			if (geojson.properties.natural == "coastline") return true
			return geojson
		});

		// BBOXの定義
		var bounds = GeoCont.get_L();
		const bbox = [bounds.NW.lng, bounds.SE.lat, bounds.SE.lng, bounds.NW.lat];

		// コーストラインのマルチラインストリングを作成
		const coastlineMultiLineString = turf.multiLineString(coastlineFeatures.map(f => f.geometry.coordinates));

		// BBOX全体を囲むポリゴンを作成
		const bboxPolygon = turf.bboxPolygon(bbox);
		GeoCont.polyline_write(bboxPolygon.geometry.coordinates)

		// コーストラインポリゴンを作成
		var coastlinePolygon = turf.lineToPolygon(coastlineMultiLineString, { properties: {} });
		coastlinePolygon = turf.unkinkPolygon(coastlinePolygon);
		//GeoCont.polyline_write(coastlinePolygon.features)

		// コーストラインポリゴンとBBOXポリゴンの差分を取って海の部分を特定
		//const seaPolygon = turf.difference(bboxPolygon, coastlinePolygon);
		const seaPolygon = turf.difference(bboxPolygon, coastlinePolygon)
		GeoCont.polyline_write(seaPolygon.geometry.coordinates)
