...
class SVGControl {
		...
		for (let i = 0; i < marker.length; i++) {
			...
			switch (pathname) {
				...
				default:
					...
					textsize = (textsize < 0) || isNaN(textsize) ? 0 : textsize;
					if (!noframe) {
	...
	svg_style(key, overstyle) {	// set svg style(no set opacity) overstyle:true時はそちらを返す
		...
		switch (overstyle) {
			case true:
				let dashs = Conf.style[key].overstyle.dashArray.split(" ");
				dashs = dashs.map(dash => dash * weight);
				common = {
					"stroke": true, "dashArray": `${dashs[0]} ${dashs[1]}`,
					"lineJoin": Conf.style[key].overstyle.linecap, "lineCap": Conf.style[key].overstyle.linecap,
					"bubblingMouseEvents": false, "weight": Conf.style[key].overstyle.width * weight,
					"color": Conf.style[key].overstyle.color
				};
				break;
			default:
				common = {
					"stroke": true, "dashArray": Conf.style[key].dashArray,
					"lineJoin": Conf.style[key].linecap, "lineCap": Conf.style[key].linecap,
					"bubblingMouseEvents": false, "weight": Layers[key].width * weight
				};
		}
		if (overstyle) {
			style = common;
		} else if (Conf.style[key].type == "area") {	// overstyle時は枠指定しない
