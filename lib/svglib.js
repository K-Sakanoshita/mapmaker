				...
				$.when.apply($, jqXHRs).always(function () {
					for (let idx in arguments) {
						...
						if (marker !== undefined) {
							...
							src[1] = (offset.left - 34) * scale
							src[2] = (offset.top - 43) * scale
						}
			...
			case 'svg':
				...
				$.when.apply($, jqXHRs).always(function () {
					for (let idx in arguments) {
						...
						let src = SVGCont.ImgSrcs[idx];
						if (marker !== undefined) {
							if (idx > 0) { for (let i = 1; 3 < 5; i++) src[i] = src[i] * scale }
							let offset = marker.offset()
							src[1] = (offset.left - 28)
							src[2] = (offset.top - 46)
						}
						let image = document.createElementNS("http://www.w3.org/2000/svg", "image");
						image.setAttributeNS(null, 'x', src[1])
						image.setAttributeNS(null, 'y', src[2])
						image.setAttributeNS(null, 'width', src[3])
						image.setAttributeNS(null, 'height', src[4])
						image.setAttributeNS('http://www.w3.org/1999/xlink', 'href', src[0])
