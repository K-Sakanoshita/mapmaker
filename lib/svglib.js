...
class SVGControl {
	...
	conv_svg(svg) {							// MakerをSVGに追加 理由：leafletがアイコンをIMG扱いするため
		...
		let parser = new DOMParser(), thumbnail, svgicon, svgtext, text, svgstl;
		SVGCont.ImgSrcs = [];
		for (let i = 0; i < marker.length; i++) {
			...
			switch (pathname) {
				...
				default:
					...
					svg_append(svg, svgicon, marker.eq(i), svgsize_circle, [-12, 0, 0, -16]);
					svgicon = $(parser.parseFromString(SVGCont.SvgIcon[filename], "text/xml")).children();					// アイコン追加
					svg_append(svg, svgicon, marker.eq(i), svgsize, [-8, 0, 0, -12]);
					text = $(marker.eq(i)[0].children).children().attr('icon-name');
					if (text !== undefined && text !== "" && Conf.effect.text.view) {					// Marker Text
						SVGCont.text_write(svg, {
							"text": text, "anchor": 'start', "x": Number(svgstl[4]) + Math.ceil(svgsize.x / 2) + 6, "y": Number(svgstl[5]) + offset[0] + 3, "no": i,
	...
	text_write(svg, params) {
		...
		rect.setAttribute("y", params.y - 3);
