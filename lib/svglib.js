...
class SVGControl {
    ...
    async save(params) {
        ...
        const exportRect = this._getExportRectPx(mode);
        console.log("export rect(px):", mode || "screen", exportRect);
        const b = this._rectToLLBounds(exportRect);
        const sw = b.SW;
        const ne = b.NE;
        ...
        // 線幅は出力解像度ではなく、画面の地図範囲から用紙への拡大率で換算する。
        const strokeMmPerPx = Math.min(
            paperMm.w / exportRect.width,
            paperMm.h / exportRect.height
        );
        const dashExportScale = Math.min(
            canvas_width / exportRect.width,
            canvas_height / exportRect.height
        );
        ...
        const exporter = new PrintSVGExporter({
            ...
            backgroundOpacity: 1,
            pxToMm: mmPerPx,
            strokePxToMm: strokeMmPerPx,
            dashScaleByLayerKey: {
                railway: 3,
                highway_alley: dashExportScale,
                highway_standard: dashExportScale,
                highway_primary: dashExportScale,
                highway_motorway: dashExportScale
            },
            minStrokeMm: 0,
        ...
        try {
            const img = await new Promise((resolve, reject) => {
                ...
                image.onerror = () => reject(new Error("PNG image load failed"));
                image.src = svgUrl;
            });
    ...
    svg_style(key, overstyle) {
        ...
        switch (overstyle) {
            case true:
                let overstyleConf = Conf.style[LayerCont.palette][key].overstyle;
                let dashs = overstyleConf.dashArray.split(" ");
                dashs = dashs.map(dash => dash * weight);
                let widthWeight = weight;
                if (nowzoom > 15 && Number.isFinite(overstyleConf.zoomWidthStep)) {
                    widthWeight = 1 + (nowzoom - 15) * overstyleConf.zoomWidthStep;
                }
                const defaultWidth = Number(Conf.style[LayerCont.palette][key].width);
                const currentWidth = Number(Layers[key].width);
                const customWidthScale = Number.isFinite(defaultWidth) && defaultWidth > 0 && Number.isFinite(currentWidth)
                    ? currentWidth / defaultWidth
                    : 1;
                common = {
                    "stroke": true, "dashArray": `${dashs[0]} ${dashs[1]}`,
                    "lineJoin": overstyleConf.linecap, "lineCap": overstyleConf.linecap,
                    "bubblingMouseEvents": false, "weight": overstyleConf.width * customWidthScale * widthWeight,
                    "color": overstyleConf.color
                };
                break;
            default:
                common = {
