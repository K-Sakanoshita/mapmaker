...
class SVGControl {
    ...
    makeSVG(svg, callback) {							// MakerをSVGに追加 理由：leafletがアイコンをIMG扱いするため
        let marker = $("div.leaflet-marker-pane").children();
        let parser = new DOMParser(), thumbnail, svgicon, svgtext, text, svgstl, imageque = []
        for (let i = 0; i < marker.length; i++) {
            ...
            switch (pathname) {
                case undefined:					// not icon(qr etc...)
                    ...
                    SVGCont.writeRect(svg, { x: x - 4, y: y - 4, width: 620, height: $(svgicon).height() + 12, opacity: 1 })	// add p-1
                    svg_append(svg, svgicon, marker.eq(i), { x: $(svgicon).width() / 2, y: $(svgicon).height() / 2 }, offset)
                    text = svgtext.children("span")[0].innerHTML
                    if (text !== undefined) {	// QRCode Text
                        x += $(svgicon).outerWidth()
                        y += 4
                        console.log("x:" + x + ",y:" + y + ",width:" + (svgtext.width() - 8))
                        SVGCont.text_write(svg, {
                            "text": text, "anchor": 'start', "x": x, "y": y, "no": i,
                            "width": svgtext.width() - 8, "height": $(svgicon).height(),
                            "size": parseInt(svgtext.css("font-size")), "color": Conf.effect.text.color, "font": "Helvetica"
                        });
                        thumbnail = $(marker.eq(i)[0].children).find("img");	// thumbnail
                        if (thumbnail !== undefined) {
                            x += (svgtext.outerWidth() - 8)
                            imageque.push({ src: thumbnail[0].src, x: x, y: y - 2, width: thumbnail.outerWidth(), height: thumbnail.outerHeight() })
                        }
                    };
                    break;
                default:
        ...
        if (imageque.length > 0) {
            ...
            imageque.forEach(param => {
                Basic.ImageToBase64(param.src, function (data) {
                    let image = document.createElementNS("http://www.w3.org/2000/svg", "image");
                    image.setAttributeNS(null, 'x', param.x)
                    image.setAttributeNS(null, 'y', param.y)
                    image.setAttributeNS(null, 'width', param.width)
                    image.setAttributeNS(null, 'height', param.height)
    ...
    save(params) {
        ...
        base = { width: box.width - (box.left + box.right), height: box.height - (box.top + box.bottom), x: base.x + box.left, y: base.y + box.top };

        if (Basic.isSmartPhone()) {
            base.x += 36;
            base.y += Conf.default.HeaderSize;
        } else {
            base.x += base.width / 10;
            base.y += base.height / 10;
        };
        ...
        SVGCont.makeSVG(svg, function () {
            scale = canvas_width / base.width;
            switch (params.type) {
                case 'png':
                    ...
                    let imgsrcs = ["data:image/svg+xml;charset=utf-8;base64," + btoa(unescape(encodeURIComponent(data))), 0, 0, base.width * scale, base.height * scale, undefined];
                    jqXHRs.push(loadImage(imgsrcs[0]))
                    $.when.apply($, jqXHRs).always(function () {
                        ctx.drawImage(arguments[0], 0, 0, arguments[0].width, arguments[0].height, 0, 0, canvas_width, canvas_height);
    ...
    writeRect(svg, params) {
        ...
        rect.setAttribute("fill-opacity", params.opacity);
    ...
    text_write(svg, params) {
        ...
        rect.setAttribute("y", params.y - (params.size - 12) * 0.6);
